...
#                      function(i) us_market[i] <- getSymbols(us_market[i], from = from
#                                                         , auto.assign = FALSE, reload.Symbols = TRUE))
# china_pool <- lapply(1:length(china_market), 
#                   function(i) china_market[i] <- getSymbols(china_market[i], from = from
#                                                          , auto.assign = FALSE, reload.Symbols = TRUE))
# renames
...
# re <- NULL
# get variable title
sapply(1:length(list), function(i){substr(list[i], 9,10)})
...
winrate_dj_arbr <- final(dj30_pool, dj30, winrate_dj_arbr)
...
stock_perform <- function(x, FUN, ...){
  ...
  print(result)
  plot(x = result[,1], y = result[,6], "l", color = "red")
}
result <- ret(perf(strat_bbands(BABA)))
interm <- perf(strat_bbands(BABA))
